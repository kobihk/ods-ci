...
Install RHODS
  ...
  ...    \- CATALOG_SOURCE: ${CATALOG_SOURCE}   console=yes
  ...
  IF   "${cluster_type}" == "selfmanaged"
      ...
      ELSE IF  "${TEST_ENV}" in "${SUPPORTED_TEST_ENV}" and "${INSTALL_TYPE}" == "OperatorHub"
          ...
          END
          Wait For Installplan And Approve It    ${OPERATOR_NAMESPACE}    ${OPERATOR_NAME}
          ...    ${OPERATOR_SUBSCRIPTION_NAME}    ${rhoai_version}
      ELSE
...
Add StartingCSV To Subscription
    ...
    ${current_starting_csv} =    Run And Return Rc And Output    oc get subscription ${OPERATOR_SUBSCRIPTION_NAME}
    ...   -n ${OPERATOR_NAMESPACE} -o jsonpath='{.spec.startingCSV}'
